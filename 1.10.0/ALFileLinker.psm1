...
function Set-ALFileLinks {
    ...
    # Copy PS Scripts - mirror central subfolder structure into PS_Scripts/
    # Repo-specific config files: copy only if missing (no overwrite)
    ...
    if ($psScriptSubDirs.Count -gt 0) {
        ...
        foreach ($psTargetBase in $psTargetFolders) {
            ...
            foreach ($subDir in $psScriptSubDirs) {
                ...
                foreach ($psFile in $centralFiles) {
                    ...
                    if ($repoSpecificFileNames -contains $psFile.Name) {
                        ...
                        # Standard file: copy from central
                ...
                foreach ($ps1File in $ps1Files) {
                    ...
                    if ($scriptContent -match '\$[Oo]ut(File|Dir|put)|\bOut-File\b') {
                        ...
                        $gitkeepFile = Join-Path $outFolder '.gitkeep'
                        if (-not (Test-Path -LiteralPath $gitkeepFile)) {
            ...
            Write-Host "Copied PS Script file(s) to: $psScriptsFolder (mirroring $($psScriptSubDirs.Count) subfolders)"
        }
    ...
    # Clean up old ALFileLinker hook and config from .git/ (from v1.7.0 and earlier)
    $oldConfigFile = Join-Path $repo '.git\alfilelinker.json'
    if (Test-Path -LiteralPath $oldConfigFile) {
        Remove-Item -LiteralPath $oldConfigFile -Force -ErrorAction SilentlyContinue
        Write-Host "Removed old ALFileLinker config: $oldConfigFile" -ForegroundColor Yellow
    }
    $oldHookFile = Join-Path $repo '.git\hooks\post-checkout'
    if ((Test-Path -LiteralPath $oldHookFile) -and ((Get-Content -LiteralPath $oldHookFile -Raw -ErrorAction SilentlyContinue) -match 'ALFileLinker')) {
        Remove-Item -LiteralPath $oldHookFile -Force -ErrorAction SilentlyContinue
        Write-Host "Removed old ALFileLinker post-checkout hook: $oldHookFile" -ForegroundColor Yellow
    }

    # Stage all copied files so they are tracked by git
...
function Set-ALFileLinksForRepos {
    ...
    $results = foreach ($repo in $repos) {
        $i++
        Write-Progress -Activity 'Copying PS scripts' -Status "$i / $total : $repo" -PercentComplete ([int](($i / [double]$total) * 100))

        $action = "Copy PS scripts from central folder"
    ...
    Write-Progress -Activity 'Copying PS scripts' -Completed
    $results
}
...
function Copy-RepoWithFileLinks {
    ...
    $result = Set-ALFileLinks -RepoPath $dest -CentralFileLinkFolder $CentralFileLinkFolder
