...
function Set-ALFileLinks {
    ...
    # Resolve subfolder paths
    $codingGuidelinesDir = Join-Path $centralDir 'Coding Guidelines'
    $psScriptsDir = Join-Path $centralDir 'PS Scripts'

    if (-not (Test-Path -LiteralPath $codingGuidelinesDir -PathType Container)) {
        throw "'Coding Guidelines' subfolder not found in: $centralDir"
    }
    ...
    $psScriptSubDirs = @()
    if (Test-Path -LiteralPath $psScriptsDir -PathType Container) {
        ...
        Write-Verbose "'PS Scripts' subfolder not found in: $centralDir (skipping)"
    }
    ...
    if ($psScriptSubDirs.Count -gt 0) {
        ...
        foreach ($subDir in $psScriptSubDirs) {
            ...
            # Detect if any script produces output files -> create out/ with .gitkeep
            ...
            foreach ($ps1File in $ps1Files) {
                ...
                if ($scriptContent -match '\$[Oo]ut(File|Dir|put)|\bOut-File\b') {
                    ...
                    $gitKeep = Join-Path $outFolder '.gitkeep'
